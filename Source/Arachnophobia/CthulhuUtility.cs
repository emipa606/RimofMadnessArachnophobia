...
namespace Arachnophobia
{
    public static class CthulhuUtility
    {
        ...
        public static bool TryGetUnreservedPewSpot(Thing pew, Pawn claimer, out IntVec3 loc)
        {
            ...
            if (claimer.CanReserve(pew.PositionHeld))
            {
                loc = pew.Position;
                return true;
            }
            ...
            {
                if (claimer.CanReserve(CellWest))
                {
                    loc = CellWest;
                    return true;
                }

                if (claimer.CanReserve(CellEast))
                {
                    loc = CellEast;
                    return true;
                }
            ...
            if (claimer.CanReserve(CellNorth))
            {
                loc = CellNorth;
                return true;
            }
        ...
        public static void TemporaryGoodwill(Faction faction, bool reset = false)
        {
            ...
            if (!reset)
            {
                ...
                faction.RelationWith(playerFaction).kind = FactionRelationKind.Neutral;
            }
            else
            {
                faction.RelationWith(playerFaction).baseGoodwill = 0;
                faction.RelationWith(playerFaction).kind = FactionRelationKind.Hostile;
            }
        }
