...
namespace Arachnophobia
{
    public class Building_Web : Building
    {
                            ...
                            else
                            {
                                for (int i = leavingsRect.minZ; i <= leavingsRect.maxZ; i++)
                                {
                                    for (int j = leavingsRect.minX; j <= leavingsRect.maxX; j++)
                                    {
                                        ...
                                        if (Rand.Value > 0.5f) FilthMaker.TryMakeFilth(c, this.Map, this.def.filthLeaving, Rand.RangeInclusive(1, 3));
                                    }
                                }
        ...
        public override void Tick()
        {
            ...
            {
                if (t is Pawn pawn && (!(pawn is PawnWebSpinner)) && !this.touchingPawns.Contains(pawn))
                {
        ...
        public override string GetInspectString()
        {
            ...
            if (Spinner != null)
            {
                str2 = (spinner.Faction != Faction.OfPlayerSilentFail) ? "ROM_Wild".Translate(spinner.Label).RawText : spinner.Name.ToStringFull;
